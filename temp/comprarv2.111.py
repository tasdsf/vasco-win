...
def focar_jogo():
    """Calcula o centro do ecrã principal e clica para focar o jogo"""
    with mss.mss() as sct:
        # No mss: monitores[0] é o total, monitores[1] é o principal
        monitor_principal = sct.monitors[1]
        
        # Calcula o meio do ecrã
        centro_x = monitor_principal["left"] + (monitor_principal["width"] // 2)
        centro_y = monitor_principal["top"] + (monitor_principal["height"] // 2)
        
        print("[SISTEMA] A focar no jogo (Ecrã Principal)...")
        # Move o rato e clica para puxar o jogo para a frente
        pydirectinput.click(int(centro_x), int(centro_y))
        time.sleep(0.5) # Dá tempo ao Windows para trazer a janela para a frente
